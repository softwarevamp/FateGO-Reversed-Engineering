...
public class PartyOrganizationConfirmDeckMenu : BaseDialog
{
    [SerializeField]
    ...
    public void Open(Kind kind, PartyListViewItem partyItem, CallbackFunc callback)
    {
        if ((this.state == State.INIT) || (this.state == State.CLOSE))
        {
            ...
            {
                case Kind.CANCEL:
                    this.titleLabel.text = LocalizationManager.Get("PARTY_ORGANIZATION_CONFIRM_CANCEL_TITLE");
                    this.messageLabel.text = LocalizationManager.Get("PARTY_ORGANIZATION_CONFIRM_CANCEL_MESSAGE");
                    this.decideLabel.text = LocalizationManager.Get("PARTY_ORGANIZATION_CONFIRM_CANCEL_DECIDE");
                    this.cancelLabel.text = LocalizationManager.Get("PARTY_ORGANIZATION_CONFIRM_CANCEL_CANCEL");
                    break;
