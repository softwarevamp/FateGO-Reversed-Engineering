...
public class UIMasterFigureRenderOld : UITweenRenderer
{
    protected static readonly int BODY_SIZE_X = 0x3fe;
    protected static readonly int BODY_SIZE_Y = 0x2fe;
    ...
    static UIMasterFigureRenderOld()
    {
        Vector2[] array = new Vector2[] { new Vector2(0f, 0f), new Vector2(0f, 0f), new Vector2(0f, 0f) };
        bodyTopTable = Array.AsReadOnly<Vector2>(array);
        Vector2[] vectorArray2 = new Vector2[] { new Vector2(1022f, 766f), new Vector2(1022f, 766f), new Vector2(1022f, 766f) };
        bodyTextureSizeTable = Array.AsReadOnly<Vector2>(vectorArray2);
        Vector2[] vectorArray3 = new Vector2[] { new Vector2(1022f, 766f), new Vector2(1022f, 766f), new Vector2(1022f, 766f) };
    ...
    public static string[] GetAssetNameList(int genderType, int equipId)
    {
        ...
        if (equipId > 0)
        {
            ...
            if (entity != null)
            {
                ...
                if (num > 0)
                {
                    strArray[0] = $"MasterFigure/equip{num:D5}";
    ...
    public Rect GetBodyUvRect()
    {
        ...
        return new Rect(vector.x / ((float) MAIN_SIZE_X), (vector.y / ((float) MAIN_SIZE_Y)) + 0.25f, vector2.x / ((float) MAIN_SIZE_X), vector2.y / ((float) MAIN_SIZE_Y));
    }
    ...
    public void SetCharacter(DispType dispType, int genderType, int equipId)
    {
        ...
        if (equipId > 0)
        {
            ...
            {
                num = (genderType != 2) ? entity.maleImageId : entity.femaleImageId;
                if ((num > 0) && !AssetManager.isExistAssetStorage($"MasterFigure/equip{num:D5}"))
                {
                    num = 0;
                }
            }
        ...
        if (this.dispType == DispType.MY_ROOM)
        {
            this.dispOffset = new Vector2((float) entity2.offsetXMyroom, (float) entity2.offsetYMyroom);
        }
        else
        {
