...
public class ServantStatusFlavorTextListViewItemDrawText : ServantStatusFlavorTextListViewItemDraw
{
    [SerializeField]
    ...
    [SerializeField]
    protected GameObject titleEquipBase;
    [SerializeField]
    protected GameObject titleServantBase;

    public override ServantStatusFlavorTextListViewItemDraw.Kind GetKind() => 
        ServantStatusFlavorTextListViewItemDraw.Kind.TEXT;
    ...
    public override void SetItem(ServantStatusListViewItem item, bool isOpen, bool isNew, string text, string text2, ServantStatusFlavorTextListViewItemDraw.DispMode mode)
    {
        ...
        if (((item != null) && (mode != ServantStatusFlavorTextListViewItemDraw.DispMode.INVISIBLE)) && this.isFirst)
        {
            ...
            if (item.Servant.IsServantEquip)
            {
                this.titleServantBase.SetActive(false);
                this.titleEquipBase.SetActive(true);
            }
            else
            {
                this.titleServantBase.SetActive(true);
                this.titleEquipBase.SetActive(false);
            }
