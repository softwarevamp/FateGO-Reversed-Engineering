...
public class ServantFramePurchaseMenu : BaseMonoBehaviour
{
    [SerializeField]
    protected ServantFramePurchaseConfirmMenu purchaseConfirmMenu;
    ...
    protected void EndBuyFrameConfirm(bool isDecide)
    {
        if (isDecide)
        {
            ...
            this.RequestServantFramePurchase();
        }
        else
        {
    ...
    protected void EndRequestServantFramePurchase(string result)
    {
        this.state = State.RESULT_BUY_FRAME;
        this.purchaseConfirmMenu.Init();
        this.purchaseConfirmMenu.Open(ServantFramePurchaseConfirmMenu.Kind.PURCHASE, this.stoneShopEntity, new ServantFramePurchaseConfirmMenu.CallbackFunc(this.EndBuyFrameResultConfirm));
    ...
    public void Open(CallbackFunc callback, System.Action refreshCallback = null)
    {
        if (this.state == State.INIT)
        {
            ...
            StoneShopEntity[] enableEntitiyList = (SingletonMonoBehaviour<DataManager>.Instance.getMasterData(DataNameKind.Kind.STONE_SHOP) as StoneShopMaster).GetEnableEntitiyList(StoneShopEffect.Kind.EXTEND_SVT_MAX);
            if (enableEntitiyList.Length <= 0)
            {
                this.state = State.INPUT_NO_SHOP_CONFIRM;
                this.purchaseConfirmMenu.Open(ServantFramePurchaseConfirmMenu.Kind.NO_INFO, null, new ServantFramePurchaseConfirmMenu.CallbackFunc(this.EndNoShopConfirm));
            }
            else
            {
                this.stoneShopEntity = enableEntitiyList[0];
                if (entity.svtKeep >= BalanceConfig.ServantFrameMax)
                {
                    this.state = State.INPUT_MAX_FRAME_CONFIRM;
                    this.purchaseConfirmMenu.Open(ServantFramePurchaseConfirmMenu.Kind.MAX_FRAME, null, new ServantFramePurchaseConfirmMenu.CallbackFunc(this.EndMaxFrameConfirm));
                }
                else if (entity.stone < this.stoneShopEntity.price)
                {
                    this.state = State.INPUT_BUY_STONE_CONFIRM;
                    this.purchaseConfirmMenu.Open(ServantFramePurchaseConfirmMenu.Kind.STONE, this.stoneShopEntity, new ServantFramePurchaseConfirmMenu.CallbackFunc(this.EndBuyStoneConfirm));
                }
                else
                {
                    this.state = State.INPUT_BUY_FRAME_CONFIRM;
                    this.purchaseConfirmMenu.Open(ServantFramePurchaseConfirmMenu.Kind.FRAME, this.stoneShopEntity, new ServantFramePurchaseConfirmMenu.CallbackFunc(this.EndBuyFrameConfirm));
                }
            }
        }
    }

    public void RequestServantFramePurchase()
    {
        if (this.stoneShopEntity != null)
        {
            NetworkManager.getRequest<PurchaseByStoneRequest>(new NetworkManager.ResultCallbackFunc(this.EndRequestServantFramePurchase)).beginRequest(this.stoneShopEntity.id, 1);
        }
        else
        {
            this.Callback(Result.ERROR);
    ...
    public delegate void CallbackFunc(ServantFramePurchaseMenu.Result result);
